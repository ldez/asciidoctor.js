...
Builder.prototype.release = function (releaseVersion) {
  ...
  async.series([
    ...
    callback => builder.completeRelease(releasePushed, callback)
  ], () => log.success(`Done in ${process.hrtime(start)[0]} s`));
};
...
if (($a = self.primary_stylesheet_data) !== false && $a !== nil && $a != null) {
  return $a;
} else {
  self.primary_stylesheet_data = Opal.const_get_relative([], "IO").$read(Opal.const_get_relative([], "File").$join(stylesheetsPath, "asciidoctor.css")).$chomp();
  return self.primary_stylesheet_data;
}
  `;
